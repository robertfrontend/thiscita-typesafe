...
type Navigation = { matches: Match[]; primaryServiceId: ServiceId | "none"; needsClarification: boolean; clarification: Clarification; urgency: boolean; source: "typesafe" | "demo"; responseLatencyMs: number };

function urgentLanguage(message: string) { return /\b(hoy|mañana|manana|urgente|vence|viajo|today|tomorrow|urgent|expires|travel)\b/i.test(message); }
function clarificationFor(message: string): Clarification { const text = message.toLocaleLowerCase(); return /pasaporte|passport/.test(text) ? "passport" : /licencia|license|permiso/.test(text) ? "license" : /veh[ií]culo|vehicle|registro|registration/.test(text) ? "vehicle" : null; }

function makeNavigation(rawMatches: Match[], source: Navigation["source"], startedAt: number, message: string, urgency = urgentLanguage(message), clarification = clarificationFor(message)): Navigation {
  ...
  return { matches, primaryServiceId, needsClarification: primaryServiceId === "none", clarification: primaryServiceId === "none" ? clarification : null, urgency, source, responseLatencyMs: Math.round(performance.now() - startedAt) };
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const response = await client.systemOne({ state: { residentMessage: message, scope: "Boston resident navigating a fixed Massachusetts and federal government service catalog" }, questions: {
      ...questions,
      urgency: noul("Does the resident explicitly indicate a near-term deadline or urgent timing? Do not infer urgency from the service alone."),
      clarification: choice("Which one clarification topic would best disambiguate this request only if it is unclear? Choose none when a route is clear.", { passport: "Whether the passport request is first-time or renewal.", license: "Whether the driver credential request is a first application, renewal, or replacement.", vehicle: "Whether vehicle registration is a first registration or renewal.", none: "The request is already clear or another clarification would be needed." }),
    } });
    const answers = response.answers as Record<string, { noul?: number; choice?: string }>;
    ...
    const clarification = candidateClarification === "passport" || candidateClarification === "license" || candidateClarification === "vehicle" ? candidateClarification : clarificationFor(message);
    return NextResponse.json(makeNavigation(matches, "typesafe", startedAt, message, Math.max(answers.urgency?.noul ?? 0, urgentLanguage(message) ? 0.9 : 0) >= 0.72, clarification));
  } catch (error) {
