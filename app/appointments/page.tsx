...
type Reply = { matches: { serviceId: string; probability: number }[]; date: string | null; time: string | null; needsService: boolean; needsDate: boolean; needsTime: boolean; source: "typesafe" | "demo"; responseLatencyMs: number };
...
export default function AppointmentsPage() {
  ...
  function applyReply(body: Reply) {
    ...
    const nextSelected = [...new Set([...selectedRef.current, ...matches])]; const nextDate = body.date ?? dateRef.current; const nextTime = body.time ?? timeRef.current;
    selectedRef.current = nextSelected; dateRef.current = nextDate; timeRef.current = nextTime;
    setSelected(nextSelected); setDate(nextDate); setTime(nextTime); setLiveSource(body.source);
  ...
  async function analyzeLive(text: string) {
    ...
    try {
      const res = await fetch("/api/interpret-appointment", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: transcript }), signal: controller.signal });
  ...
  async function send(text: string) {
    ...
    try {
      const res = await fetch("/api/interpret-appointment", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: text }) });
      ...
      const reply = `${names ? `Agregué: ${names}. ` : ""}${missing.length ? `Dime ${missing.join(" y ")}.` : `Listo: ${nextSelected.length} servicio${nextSelected.length === 1 ? "" : "s"} para ${readableDate(nextDate)} a las ${nextTime}.`}`;
  ...
  function speak() {
    ...
    finalTranscriptRef.current = ""; visibleTranscriptRef.current = ""; lastAnalyzedTranscriptRef.current = ""; lastLiveCallAtRef.current = 0; setMessage(""); setError(""); setLiveSource(null);
