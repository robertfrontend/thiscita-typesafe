...
export default function ProposalsPage() {
  ...
  async function analyze(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (!document) return;
      const form = new FormData(); form.append("document", document);
  ...
  return <main className="proposal-page"><header className="masthead"><a className="brand" href="/"><span>OBRA</span><i>β</i></a><span className="demo-tag">Módulo 02 · {t.badge}</span><a className="proposal-back" href="/">← {t.back}</a><button className="language" type="button" onClick={() => setLocale(locale === "es" ? "en" : "es")}>{locale === "es" ? "English" : "Español"}</button></header>
    ...
    <section className="proposal-workspace"><form className="proposal-editor" onSubmit={analyze}><div className="proposal-editor-head"><label htmlFor="proposal-document">{t.label}</label><span>{t.formats}</span></div><label className="document-drop" htmlFor="proposal-document"><input id="proposal-document" type="file" accept=".pdf,.docx,.txt,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,text/plain" onChange={(event) => setDocument(event.target.files?.[0] ?? null)} /><span className="document-icon" aria-hidden="true">↥</span><strong>{document ? document.name : t.choose}</strong><small>{document ? `${Math.ceil(document.size / 1024)} KB · ${t.change}` : t.formats}</small></label><div className="proposal-actions"><small>{t.private}</small><button type="submit" disabled={loading || !document}>{loading ? t.analyzing : t.analyze}</button></div>{error && <p className="error" role="alert">{error}</p>}</form>
